...
$sqlEstado="SELECT DISTINCT h.`ip_tablet`,c.`estadoProducto` FROM `habitaciones`= h INNER JOIN carritos =c ON c.habitacion=h.ip_tablet";
...
<body style="background: #3C3B3BFE;">
  ...
  <div style="width: 95%;margin-left: 2%;background: #0c63d06b;" class="card">
      ...
      <h2 style="color: #43d9ff;font-size:350%;"><?php echo $nombre?></h2>
        ...
        <?php 
        if ($pedido['categoria']!="12") {
        ?>
        <h3><?php echo $pedido['nombre']." X".$pedido['cantidad'];?><a href="moduloCocina/eliminarPedidoCarrito.php?idCarrito=<?php echo $pedido['idCarrito'];?>&ip=<?php echo $_GET['ip']."&habitacion=".$nombre;?>" style="" class="btn btn-sm btn-danger"><i class="fas fa-trash-alt"></i></a></h3>
        <?php }?>
        <?php endforeach;?>
          <a href="moduloCocina/listo.php?ip=<?php echo $_GET['ip']."&habitacion=".$nombre?>" class="btn btn-success">listo</a>
          <a href="moduloTicket/index.php?ip=<?php echo $_GET['ip']."&habitacion=".$nombre?>" class="btn btn-success">Imprimir ticket</a>
        <?php }?>
...
foreach ($habitaciones as $key):
        ...
        foreach ($estado as $esta) {
          if ($esta['ip_tablet']==$key['ip_tablet']) {
              ...
              if ($esta['estadoProducto']=="cocina") {

                
                $boton.="<a href='cocina.php?ip=".$key['ip_tablet']."&nombre=".$key['nombre']."' id=".$key['ip_tablet']." class='btn colorAmarillo' style='background-color:#00c851;color:white;'>".$key['nombre']."</a>";
                if($key['cocina']==""){
                $si="si";
                $cocinaSql="UPDATE `habitaciones` SET `cocina`=:si WHERE `habitacion`=:habitacion";
                $executeCocina=$conexion->prepare($cocinaSql);
                $executeCocina->bindParam(":si",$si);
                $executeCocina->bindParam(":habitacion",$key['habitacion']);
                $executeCocina->execute();
        ...
        if ($vandera==1) {
          ...
          $boton.="<a href='cocina.php?ip=".$key['ip_tablet']."&nombre=".$key['nombre']."' id=".$key['ip_tablet']." class='btn disabled' style='background-color:#343535;color:white;'>".$key['nombre']."</a>";
        }
        $vandera=1;
...
localStorage.setItem("cocina", <?php echo json_encode($arrayCocina)?>)

console.log(JSON.parse(localStorage.getItem("cocina")))


JSON.parse(localStorage.getItem("cocina")).forEach(async (element)=> {
  console.log(element)
